...
class Input extends InputGeneric {
  ...
  constructor() {
    ...
    this.map[19] = Input.CODE_PAUSE; // no in RC

    this.map[13] = Input.CODE_ENTER;
    this.map[8] = Input.CODE_BACK;
    this.map[1003] = Input.CODE_MENU;

    this.map[112] = Input.CODE_RED; // F1
    this.map[113] = Input.CODE_GREEN; // F2
    this.map[114] = Input.CODE_YELLOW; // F3
    this.map[115] = Input.CODE_BLUE; // F4
    this.map[116] = Input.CODE_VOLUME_DOWN; // F5
    this.map[117] = Input.CODE_VOLUME_UP; // F6
    this.map[118] = Input.CODE_MUTE; // F7
    ...
    document.addEventListener('backbutton', Input.onBackButton, false);
    document.addEventListener('menubutton', Input.onMenuButton, false);
  }


  static onBackButton() {
    let event = new KeyboardEvent('keydown');
    Object.defineProperty(event, 'keyCode', { value: 8 });
  ...
  static onMenuButton() {
    let event = new KeyboardEvent('keydown');
    Object.defineProperty(event, 'keyCode', { value: 1003 });
    document.body.dispatchEvent(event);
    event = new KeyboardEvent('keyup');
    Object.defineProperty(event, 'keyCode', { value: 1003 });
    document.body.dispatchEvent(event);
