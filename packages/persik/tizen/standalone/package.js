...
const OUTPUT_WGT = 'persik3.tizen.wgt';
const DIR = __dirname;
...
async function build() {
  const CERT_FILE = 'ifaced.p12';
  const CERT_PASS = 'ifaced';
  const SEC_PROFILE = 'persiktv';
  ...
  if (TIZEN_CLI) {
    ...
    await fse.copy(__dirname + '/src/.buildResult/Persik.TV.wgt', __dirname + `/dist/${OUTPUT_WGT}`);
    process.stderr.write(' OK');
